...
export class Play extends Phaser.Scene {
    ...
    private cardNames: string[] = ["card-0", "card-1", "card-2", "card-3", "card-4", "card-5"];
    private cards: any[] = [];
    ...
    startGame(): void {
        ...
        const winnerText = this.add.text(this.sys.game.scale.width / 2, -1000, "ПОБЕДА!",
            { align: "center", strokeThickness: 4, fontSize: 40, fontStyle: "bold", color: "#8c7ae6" }
        ).setOrigin(.5)
            .setDepth(3)
            .setInteractive();
            ...
            if (this.canMove && this.cards.length) {
                ...
                if (card) {
                    ...
                    if (this.cardOpened !== undefined) {
                        ...
                        // Flip card opened and update the history of opened card
